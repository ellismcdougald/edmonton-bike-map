...
const dbConfig = {
	...
	host: 'localhost',
	port: parseInt(process.env.POSTGRES_TEST_PORT || '5435')
};
...
test.describe('change password', () => {
	...
	test('changing password fails when new passwords do not match', async ({ page }) => {
		await page.goto('http://localhost:4173/');
		...
		await expect(page.locator('text=New passwords do not match')).toBeVisible({ timeout: 5000 });
